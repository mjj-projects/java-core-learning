...
```
共享空数组实例，被用在指定初始容量为0的构造函数中，即`new ArrayList(0)`。
...
```
共享空数组实例，被用在无参构造函数中，即`new ArrayList()`。
...
private int size;
```
记录`ArrayList`实际包含的元素数量，即`list.size()`方法返回的值。
1. 当 `size == elementData.length`时，表示数组已满下次添加将触发扩容。
2. 添加：新元素被放在 `elementData[size]`，然后 `size++`。
3. 删除：移除元素后，后续元素前移，然后 `size--`。
...
```
`ArrayList`的核心数组结构，用于存储所有元素的数组缓冲区。
